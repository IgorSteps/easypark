...
func TestGetSingleAlertHandler_ServeHTTP_HappyPath(t *testing.T) {
	// --------
	// ASSEMBLE
	// --------
	testLogger, _ := test.NewNullLogger()
	mockFacade := &mocks.AlertFacade{}
	handler := handlers.NewAlertGetSingleHandler(testLogger, mockFacade)
	...
	mockFacade.EXPECT().GetAlert(req.Context(), testID).Return(testAlert, nil).Once()
	...
	assert.Equal(t, testAlert, actualAlert, "Alerts don't match")
